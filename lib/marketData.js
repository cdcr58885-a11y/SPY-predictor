...
export async function getAllMarketData(ticker = "SPY") {
  ...
  const symbolMap = { "SPX": "%5EGSPC", "NDX": "%5EIXIC", "VIX": "%5EVIX" };
  ...
  // Fetch historical data (3mo 1d) for technicals + recent 5m for intraday chart
  const [mainData, recentData, vixData, tnxData, brentData] = await Promise.all([
    fetchYahoo(mainSymbol, "3mo", "1d", false),
    fetchYahoo(mainSymbol, "1d", "5m", usePrePost), // 5min intraday - ~78 points, perfect for mobile
  ...
  // Previous day close = second to last in daily data
  const prevPrice = parseFloat(prev(close).toFixed(2));
  // Current price = latest 5m bar
  ...
  // Today's open = first bar of the 5m intraday data
  const recentOpen = clean(recentData.indicators.quote[0].open);
  const todayOpen = recentOpen.length ? parseFloat(recentOpen[0].toFixed(2)) : prevPrice;
  const todayHigh = recentHigh.length ? parseFloat(Math.max(...recentHigh).toFixed(2)) : parseFloat(last(high).toFixed(2));
  const todayLow  = recentLow.length ? parseFloat(Math.min(...recentLow.filter(v => v > 0)).toFixed(2)) : parseFloat(last(low).toFixed(2));
