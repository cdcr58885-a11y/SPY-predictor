...
export async function fetchYahoo(symbol, range = "1mo", interval = "1d") {
...
export async function getAllMarketData(ticker = "SPY") {
  ...
  // Map special tickers to Yahoo Finance symbols
  const symbolMap = { "SPX": "%5EGSPC", "VIX": "%5EVIX" };
  const mainSymbol = symbolMap[ticker] || ticker;

  const [mainData, vixData, tnxData, brentData] = await Promise.all([
    fetchYahoo(mainSymbol, "3mo", "1d"),
