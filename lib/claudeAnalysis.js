...
export async function getClaudePrediction(marketData) {
  ...
  const prompt = `You are a senior quantitative analyst. Analyze the following real-time market data and predict the S&P 500 Index (SPX)'s NEXT trading session direction.

=== CURRENT SPX DATA ===
Price:  ${price}  (${change >= 0 ? "+" : ""}${change}, ${changePct >= 0 ? "+" : ""}${changePct}%)
Open:   ${open}   High: ${high}   Low: ${low}
Volume: N/A (SPX is an index)
